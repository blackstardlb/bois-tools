...
class OutfitMemberService(
    ...
    suspend fun getOutfitMemberRankChangeRecommendations(tag: String): List<OutfitMemberRankRecommendation> {
        return outfitCensusRepository.getFullOutfitMembersForTag(tag)
                .asSequence()
        ...
        return outfitCensusRepository.getOutfitMemberOrbitalStrikeFactionStats(tag).asFlow()
                .flatMapMerge(concurrency = 5) { strikeFactionStats ->
                    flow {
                        ...
                        val hits = strikeFactionStats.orbitalStrikeHitAndScoreStats.firstOrNull { it.id == "weapon_hit_count" }?.value
                                ?: 0L
                        val score = strikeFactionStats.orbitalStrikeHitAndScoreStats.firstOrNull { it.id == "weapon_score" }?.value
                                ?: 0L
...
fun FullOutfitMember.toOutFitMembersStats(): OutfitMemberStats {
    ...
    val deaths = this.charactersStatHistory.first { it.id == "deaths" }.week.values.sum()
